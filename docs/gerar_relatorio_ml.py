...
class RelatorioPDF(FPDF):
    """Documento em fonte serifada (Times), com margens generosas e formatação mínima."""
    ...
    def paragrafo(self, texto: str) -> None:
        ...
        self.multi_cell(w = 0, h = 6.5, text = texto)
        self.ln(2)

    def item_lista(self, texto: str) -> None:
        self.set_font("Times", "", 12)
        self.set_x(self.l_margin + 6)
        self.multi_cell(w = self.epw - 6, h = 6.5, text = f"- {texto}")
        self.ln(1)
...
def gerar_relatorio() -> None:
    ...
    pdf.item_lista("MOODY, G. B.; MARK, R. G. The impact of the MIT-BIH Arrhythmia Database. IEEE Engineering in Medicine and Biology Magazine, v. 20, n. 3, p. 45-50, 2001.")
    pdf.item_lista("DE CHAZAL, P.; O'DWYER, M.; REILLY, R. B. Automatic classification of heartbeats using ECG morphology and heartbeat interval features. IEEE Transactions on Biomedical Engineering, v. 51, n. 7, p. 1196-1206, 2004.")
    pdf.item_lista("ASSOCIATION FOR THE ADVANCEMENT OF MEDICAL INSTRUMENTATION (AAMI). ANSI/AAMI EC57: Testing and reporting performance results of cardiac rhythm and ST segment measurement algorithms. 1998/2012.")
    pdf.item_lista("KACHUEE, M.; FAZELI, S.; SARRAFZADEH, M. ECG Heartbeat Classification: A Deep Transferable Representation. In: IEEE International Conference on Healthcare Informatics (ICHI), 2018.")
    pdf.item_lista("PEDREGOSA, F. et al. Scikit-learn: Machine Learning in Python. Journal of Machine Learning Research, v. 12, p. 2825-2830, 2011.")
